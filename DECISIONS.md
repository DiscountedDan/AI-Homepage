...
- **Fixed height, not max-height, for the To-Do scroll container** — `max-height` lets the box visibly grow with each added task up to the cap; since the four Home widgets sit in a CSS grid with default stretch-alignment, that growth also stretched sibling cards in the same row. `.todo-scroll` now uses a fixed `height: 210px` so the card (and row) size never changes with task count
